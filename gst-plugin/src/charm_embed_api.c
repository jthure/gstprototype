...
{
	if (pObject1 == NULL)
		return NULL; // || pObject2 == NULL) return NULL;
...
{
	PyObject *pFunc, *pValue, *pArgs, *pKWArgs, *o = NULL, *l = NULL;
	char *fmt, *list, *list2, *token, *token2, *key;
	char delims[] = "[,]";
	va_list arg_list;

	if (pObject == NULL)
		return NULL; /* can't do anything for you */
	pArgs = PyList_New(0);
	pKWArgs = PyDict_New();
	...
	for (fmt = types; *fmt != '\0'; fmt++)
	{
		...
		{
		case 'b':
			o = PyBytes_FromString(va_arg(arg_list, char *));
		...
		case 's':
			if (kw == 0)
			{
				o = PyUnicode_FromString(va_arg(arg_list, char *));
				PyList_Append(pArgs, o);
				Free(o);
			}
			else
			{
				key = va_arg(arg_list, char *);
				o = PyUnicode_FromString(va_arg(arg_list, char *));
				PyDict_SetItemString(pKWArgs, key, o);
				Free(o);
		...
		case 'I':
			o = PyLong_FromLong(atoi(va_arg(arg_list, char *)));
			PyList_Append(pArgs, o);
			Free(o);
			break;
		case 'i':
			o = PyLong_FromLong((long)va_arg(arg_list, int *));
			PyList_Append(pArgs, o);
			Free(o);
			break;
	...
	if (pFunc && PyCallable_Check(pFunc))
	{
		...
		pValue = PyObject_Call(pFunc, PyList_AsTuple(pArgs), pKWArgs);
		if (pValue == NULL)
		{
			if (PyErr_Occurred())
				PyErr_Print();
